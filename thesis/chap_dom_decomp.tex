...
In the above equations, the number of flow paths connecting a NBC volume to other continuity volumes within the linear domain is given by $N_{f}$.
...
Once \eqref{eqn:linearPressureMatrix} has been inverted, the resulting pressure updates, $\delta \vec{P}_{l}$, for the linear domain are functions of the all of the unknown flow rates between the domains, as shown in \eqref{eqn:linearDomainPressureUpdates}.
...
\delta P_{j} + \sum_{i\,=\,1}^{N_{n}} \vec{w}^{T}_{j, i} \cdot \delta \vec{\Psi}_{i} = \delta P_{j}^{*} - \sum_{i\,=\,1}^{N_{n}} \vec{w}^{T}_{j, i} \cdot{} \vec{\Psi}^{k}_{i}
\end{equation}

\comment{I need to define the deltap star.}
...
This interpretation means that \eqref{eqn:linearDomainPressureUpdates} is interpreted not as an iteratively solution but as a change from old-time to new-time values.
...
\begin{IEEEeqnarray}{rcl}
\delta P_{j} + \sum_{i\,=\,1}^{N_{n}} \vec{w}^{T}_{j,i} \vec{\Xi}^{k}_{i} \delta \vec{m}^{k}_{i} & = & \delta P^{*}_{j} - \sum_{i\,=\,1}^{N_{n}} \vec{w}^{T}_{j,i} \vec{\Xi}_{i}^{k} \momVec{}^{k}_{i}  \nonumber \\
\delta P_{j} + \sum_{i\,=\,1}^{N_{n}} \vec{w}^{T}_{j,i} \left[\vec{\Xi}_{i}^{k} \frac{\partial \momVec{}_{i}^{k}}{\partial P_{s(i)}} \delta P_{s(i)} + \vec{\Xi}_{i}^{k} \frac{\partial \momVec{}_{i}^{k}}{\partial P_{o(i)}} \delta P_{o(i)}\right] & = & \delta P^{*}_{j} - \sum_{i\,=\,1}^{N_{n}} \vec{w}^{T}_{j,i} \left[ \vec{\Xi}_{i}^{k} \momVec{}^{k}_{i} + \vec{\Xi}_{i}^{k}\delta \momVec{}^{*}_{i} \right] \nonumber \\
\label{eqn:nbcPressureEquation}
\delta P_{j} + \sum_{i\,=\,1}^{N_{n}} \vec{w}^{T}_{j,i} \left[\vec{\Xi}_{i}^{k} \frac{\partial \momVec{}_{i}^{k}}{\partial P_{s(i)}} \delta P_{s(i)} + \vec{\Xi}_{i}^{k} \frac{\partial \momVec{}_{i}^{k}}{\partial P_{o(i)}} \delta P_{o(i)}\right] & = & \delta P^{*}_{j} - \sum_{i\,=\,1}^{N_{n}} \vec{w}^{T}_{j,i} \vec{\Xi}_{i}^{k} \momVec{}^{k + \onehalf}_{i}
\end{IEEEeqnarray}
...
The subscript $o(i)$ represent the nonlinear continuity volume to which the flow path $i$ is connected.
...
\delta P_{j} = \delta P^{*}_{j} - \sum_{i\,=\,1}^{N_{n}} \vec{w}_{j,i}^{T} \vec{\Xi}^{k}_{i} \left[ \momVec{}^{k}_{i} + \delta \momVec{}_{i}\right]
\end{equation}
...
In this section, several architectural changes that were necessary to integrate the domain decomposition algorithm in \sect{sec:domDecompMath} are discussed and outlined.
...
Second, each continuity volume is designated as either a regular \classname{volume} or an \classname{nbcVolume}.
...
From \sect{sec:domDecompMath} and \eqref{eqn:linearDomainPressureUpdates}, each NBC volume has an associated pressure equation, \eqref{eqn:nbcPressureEquation}.
This equation represents the inter-dependency of the NBC volumes upon each other.
Evaluating \eqref{eqn:nbcPressureEquation} requires an inner loop over all of the NBC volumes for each outer NBC volume.

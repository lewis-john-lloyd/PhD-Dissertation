...
&+& \dt{}\left[(1-\eta)\Gamma + \Upsilon \right]^{n+1, k} +  \dt{} \sum_{p\,\in \, N_{\text{NBC}} } \left( \Psi^{n+1, k}_{c,l} \right)_{p}  \\
...
&-& \dt{}\left[ \Upsilon -\eta\Gamma \right]^{n+1,k} +  \dt{} \sum_{p \, \in \, N_{\text{NBC}} } \left( \Psi^{n+1, k}_{c,e} \right)_{p}\\
...
\fig{fig:matrixClassDiagram} shows the class diagram for the \classname{matrix} heirachy.
...
Once the \classname{matrix} heirachy was developed, an abstract \classname{solver} class was designed.
...
\fig{fig:solverClassDiagram} shows the class diagram for the \classname{solver} heirachy.

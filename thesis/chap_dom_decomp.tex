...
The nonlinear input file described \sect{subsect:nlnCobraInputFile} has the following appended to it:
...
Given that the domain was now being broken into two units, the original memory storage for volumes became cumbersome.
It was determined that the path forward would embrace the object-oriented lifestyle.
For the reasons outlined in the solver section.
\comment{here be a reference, arggh!}
There are two types of volumes: those that inhabit a single domain and those which are in the linear domain but have a flowpath connecting the to nonlinear domain.

...
&+& \dt{}\left[(1-\eta)\Gamma + S \right]^{n+1, k} +  \dt{} \sum_{p\,\in \, N_{\text{NBC}} } \left( \Psi^{n+1, k}_{c,l} \right)_{p}  \\
...
&-& \dt{}\left[ S -\eta\Gamma \right]^{n+1,k} +  \dt{} \sum_{p \, \in \, N_{\text{NBC}} } \left( \Psi^{n+1, k}_{c,e} \right)_{p}\\
...
\begin{IEEEeqnarray}{rcl}
\delta \vec{P}_{l} + \vec{A}^{-1}_{l}\vec{B}_{l} \delta \vec{\Psi}_{l} & = & \vec{A}^{-1}_{l}\vec{res}_{l} - \vec{A}^{-1}_{l}\vec{B}_{l} \vec{\Psi}_{l} \nonumber \\
\label{eqn:invertedLinearSystem}
\delta \vec{P}_{l} + \vec{W}_{l} \delta \vec{\Psi}_{l} & = & \delta \vec{P}^{*}_{l} - \vec{W}_{l} \vec{\Psi}_{l}
\end{IEEEeqnarray}
...
In this section, several architectural changes that were necessary to integrate the domain decomposition algorithm in \sect{sec:domDecompMath} are addressed.
...
\fig{fig:matrixClassDiagram} shows the class diagram for the \classname{matrix} \comment{ecosystem}.
...
Once the \classname{matrix} \comment{ecosystem} was developed, an abstract \classname{solver} class was designed.
...
\fig{fig:solverClassDiagram} shows the class diagram for the \classname{solver} \comment{ecosystem}.
...
The \classname{baseVolume} also contains the Jacobian matrix and the right-hand sides from \sect{sect:linCobraAlg}.
...
A \classname{nbcVolume} will have more columns in \classname{jac} and \classname{b} than a comparable \classname{baseVolume}.  
...
The manner in which the domain decomposition's mathematical algorithm provided in \sect{sec:domDecompMath} is implemented will now be covered.

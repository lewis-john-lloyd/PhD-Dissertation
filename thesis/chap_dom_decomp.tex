...
Each momentum volume is associated with a single domain; the momentum equations are not modified, regardless of the domain in which they exists.
The coupling between the two domains occurs at the interfaces between the linear and the nonlinear domains.
Those continuity volumes which are in the linear domain, but have flowpaths connecting them to the nonlinear domain, are considered to be nonlinear boundary volumes.
...
The convention in \cobra{} is that the normal vector for a given volume will be outwardly directed.
...
In addition, there will now be $N_{\text{NBC}}$ identity matrices corresponding to the domain boundaries associated with the given continuity volume.
...
The linear system for the nonlinear boundary volume is then subjected to partial LU decomposition without pivoting.
...
\begin{itemize}
\item{ \classname{insert} : a procedure for inserting a continuity volume's pressure equation into $\vec{A}_{i, :}$.}

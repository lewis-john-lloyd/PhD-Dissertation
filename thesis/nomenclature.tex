...
\nomtypeR{$s_{m}$}{Source or sink of mass in conversation of mass equations.}{$\lbm{}\,\text{s}^{-1} $}
\nomtypeR{$w^{n+1}_{k, j}$}{Volumetric flux for phase k in \relap53d{}.}{$ \text{ft}^{3}\,\text{s}^{-1}$}
\nomtypeR{$u^{n+1}_{k, j}$}{Flux of internal energy for phase k in \relap53d{}.}{$\text{BTU}\,\text{s}^{-1} $}
\nomtypeR{$\vec{u}$}{Velocity vector for a given flow path.}{$ \text{ft}\, \text{s}^{-1}$}
...
\nomtypeR{$m^{n+1}_{k,j}$}{Flux of mass for phase k in \relap53d{}.}{$ \lbm{}\, \text{s}^{-1} $}

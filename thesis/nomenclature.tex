...
\nomtypeR[A]{$\tilde{A}$}{Effective cross-sectional area between two momentum flow paths.}{$ \text{ft}^{2} $}
\nomtypeR{$\tilde{a}$}{Variable $a$ averaged of a given cross-section.}{}
\nomtypeR{$s_{m}$}{Source or sink of mass for a given field or phase k}{$\lbm{}\,\text{s}^{-1} $}
...
\nomtypeR{$q_{i,\phi}$}{Energy transferred between the saturated interface and any given field or phase $\phi$.}{$ \text{BTU}\,\text{ft}^{-3} \text{s}^{-1}$}
...
\nomtypeR{$s_{p}$}{External source or sink of momentum.}{$ \lbf{} \, \text{ft}^{-3} $}
...
\nomtypeR{$q_{w,\phi}$}{Energy transfer rate between a solid structure and any given field or phase $\phi$}{$ \text{BTU} \, \text{s}^{-1}$}
\nomtypeR{$s_{e}$}{Rate of energy transfer from an external source or sink.}{$ \text{BTU} \, \text{s}^{-1} $}
...
\nomtypeR{$N_{\text{NBC}}$}{Set of inter-domain connection for a given linear continuity volumes.}{}
...
\nomtypeR{$\vec{U}$}{Upper triangular matrix from \vec{LU} decomposition of a given continuity volume.}{}
\nomtypeR{$\vec{L}$}{Lower triangular matrix from \vec{LU} decomposition of a given continuity volume.}{}
\nomtypeR{$S$}{Scale factor for an equation.}{}
\nomtypeR{$\vec{A}$}{A given pressure matrix.}{}
...
\nomtypeR{$\vec{B}$}{Matrix representing global pressure matrix inter-domain coupling coefficients for the flow rates.}{}
...
\nomtypeG{$\delta \vec{x}$}{Newton update vector}{}
\nomtypeG{$\tau^{'}_{w,\phi}$}{Shear force from contact between the channel wall and field or phase k}{$\lbf{} \, \text{ft}^{-3}$}
\nomtypeG{$\tau^{'}_{i,\phi_{1} \phi_{2}}$}{Shear force from contact between the field or phase $k_{1}$ and $k_{2}$}{$ \lbf{} \, \text{ft}^{-3} $}
...
\nomtypeG{$\Gamma h^{'}_{\phi}$}{Energy transfer rate due to aqueous phase change}{$ \text{BTU} \, \text{s}^{-1} $}
...
\nomtypeT{$'''$}{Volumetric derivative of accented variable.}

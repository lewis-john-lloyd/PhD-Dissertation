...
\nomtypeR{$s_{h}$}{Source or sink of external energy.}{$ \text{BTU} $}
...
\nomtypeR{$q_{i,\phi}$}{Energy transferred between the saturated interface and a phase $\phi$.}{$ \text{BTU}$}
...
\nomtypeR{$\dot{m}_{\phi}$}{The conserved quantity for a momentum field or phase equation.}{$\lbm{}\,\text{s}^{-1}$}
\nomtypeR{$\dot{m}(t)$}{Specified mass flow at boundary.}{$\lbm{}\,\text{s}^{-1}$}
...
\nomtypeR{$F_{\text{tol}}$}{Convergence tolerance with respect to the scaled residual vector.}{}
\nomtypeR{$\kmax{}$}{Maximum iteration count for nonlinear solver.}{}

\nomtypeG{$\delta \vec{P}$}{Pressure updates for a domain.}{}
\nomtypeG{$\delta \vec{\Psi}$}{Flow rate updates.}{}
\nomtypeG{$\dtmax{}$}{Maximum timestep allowed for a simulation.}{$\text{s}$}
\nomtypeG{$\dtcrnt{}$}{Timestep as calculated by CFL limit.}{$\text{s}$}
\nomtypeG{$\dt{}_{\text{vol}}$}{Timestep as calculated by volume CFL limit.}{$\text{s}$}
...
\nomtypeG{$\Gamma \vec{u}^{'}$}{Momentum transfer due to the exchange of mass between the aqueous phases.}{$ \lbf{}\, \text{s} $}
\nomtypeG{$\Upsilon \vec{u}^{'}$}{Momentum transfer due to the exchange of mass between the two liquid fields.}{$ \lbf{} \, \text{s}$}
...
\nomtypeG{$\Omega$}{The domain; the set of volumes that comprises the domain.}{}
...
\nomtypeT{p}{Generic exponent.}
\nomtypeT{~}{Scaled vector.}
...
\nomtypeS{p}{Relating to a momentum flow path.}
\nomtypeS{c}{Relating to a continuity volume.}
\nomtypeS{m}{Relating to a continuity volume's mass equations.}
\nomtypeS{h}{Relating to a continuity volume's energy equations.}
...
\nomtypeS{ana}{Analytic.}
\nomtypeS{j}{The coordinate of a particular location in a given problem.}
\nomtypeS{o}{Base value.}
\nomtypeS{*}{An intermediate or tentative value.}
\nomtypeS{cbv}{Continuity boundary volume between \relap53d{} and \cobra{}.}
\nomtypeS{i}{Index representing a generic coordinate.}
\nomtypeS{o(i)}{Coordinate of other continuity volume connected via a flow path at coordinate i.}
\nomtypeS{s(i)}{Coordinate of current continuity volume connected via a flow path at coordinate i.}
...
Each continuity and momentum volume has an associated cross-sectional area, $A_{c, j}$ and $A_{m,j}$. 
...
Additionally, it is permissible to have the flow from a single channel split into multiple channels in an adjacent section.
...
While this designation comes from the three water fields that are modeled by the software, there software also modeled dissolved \ncg{} fields.
...
\item{
The effects of turbulence are neglected in this work.
}
\end{itemize}
...
Two momentum volumes, $j + \onehalf $ and $j - \onehalf$, spatially overlap a continuity volume, $j$.
Variables are indexed by the mesh on which their conservation equations are defined.
For example, the velocity $u_{j - \onehalf}$ would be spatially located at center of the momentum volume $j - \onehalf $ and at the boundary between the continuity volumes $j$ and $j-1$.
...
The cross-sectional area for a momentum volume, $A_{m,j + \onehalf}$, can be defined independently of the two cross-sectional areas of the adjoining continuity volumes.
...
-\left[<\alpha_l \rho_l u_l>_{d} u_l \tilde{A}\right]_{x_{j }}^{x_{j + 1}}
\end{equation}
...
Second, the velocity that is used to determine the donored quantities is the arithmetic mean of the velocities from the two adjacent momentum volumes, \eqref{eqn:average_advecting_vel}.

\begin{equation}
\label{eqn:average_advecting_vel}
u_{k,j} = \frac{1}{2}\left(u_{k,j - \onehalf} + u_{k, j+ \onehalf}\right)
\end{equation}
...
The definition of the conserved momentum quantity, $\dot{m}_k$, is given by \eqref{eqn:mom_dot}.

\begin{equation}
\label{eqn:mom_dot}
\dot{m}_{k} = \ave{\alpha_k \rho_k}_{\text{a}} u_{k} A_{m}
\end{equation}
...
\ave{a}_{a,j + \onehalf} = \frac{a_{j} + a_{j+1}}{2}
\end{equation}
...
That weakness is the Courant-Friedrichs-Lewy (CFL) limit imposed upon the timestep size, $\Delta t$.
...
\begin{equation}
\label{eqn:si_vel}
u^{n+1}_{k, j \pm \onehalf} = \frac{\dot{m}^{n+1}_{k, j \pm \onehalf}}{A_{m, j \pm \onehalf} \ave{\alpha_{k} \rho_{k}}^{n}_{\text{a}, j \pm \onehalf}} 
\end{equation}
...
The pressure update for a given continuity volume at the boundary of the master domain, $\delta P_{\text{cbv}}^{n+1}$, is expressed in terms of the unknown velocities in the slave momentum volumes adjacent to the boundary of the master continuity volumes, \eqref{eqn:pressure_coupled} \cite{Weaver2002}.
...
This unique use of domain decomposition for selective nonlinear-refinement via semi-implicit coupling may provide a route to obtaining nonlinearly-converged, timestep-size insensitive solutions for traditional two-phase flow methods with a lower computational cost than fully iterative Newton methods.

...
In order to determine the effects of nonlinear nonconvergence upon the temporal convergence of a solution, the \cobra{} software was modified to enable an iterative Newton method.
A metric was developed to detect the presence of a nonlinearly nonconverged time-step size insensitive simulation results.
A dynamic scaling for the nonlinear residual needed to be developed that would equally weight each of the conservation equation. 
Test problems were developed to show that the metric for nonlinear nonconvergence can identify situations where the temporally-converged simulation results may be inaccurate.
...
\comment{Why did the single-shot linearization fail to run?
I couldn't say mathematically.}

Note that the nonlinear \cobra produced a solution that was qualitatively different than semi-implicit, \fig{fig:flashing_solution}.
...
This is in contrast to the nonlinear flashing problem, which shows a lower residual through out, \fig{fig:nonlinear_flashing_residual}.
...
A operator-based scaling was introduced and implemented into the nonlinear \cobra{}.
Deficiencies were been identified with the current implementation of the operator-based scaling.
A metric to detect nonlinear-convergence of a temporally converged simulation was developed and implemented.
Tests indicated that an nonlinearly non-converged time-step size insensitive solution may, in fact, not be the correct solution to the nonlinear discrete equations.
It was found that the metric to determine nonlinear non-convergence of solutions provided a way to determine if the solution obtained may not be the correct solution. 
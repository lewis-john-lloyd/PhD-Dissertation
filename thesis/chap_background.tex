...
Next, the various solution methods used to solve the system of discrete governing equations are presented.
And last, the various means by which domain coupling of thermal-hydraulic safety analysis software has traditionally been achieved are presented.
...
The computational modeling framework of \cobra{} involves two primary components: sections and channels.
...
Each continuity volume and momentum flow path has an associated cross-sectional area, $A_{c, j}$ and $A_{m,j}$, respectively. 
...
For the conditions encountered in the a nuclear reactor core, certain assumptions can be made to reduce the complexity of the physics involved.
...
\tilde{a} = \frac{1}{A}\int_{A} a \;\mathrm{dA}
\end{equation}
...
\frac{\partial \left(\alpha_g \rho_v \right)}{\partial t } + \nabla \cdot \left( \alpha_g \rho_v \vec{u}_g \right)         & = & \dot{\Gamma}^{'''} + s_{m,v} \\
\label{eqn:conservation_of_liq}
\frac{\partial \left(\alpha_l \rho_l \right)}{\partial t } + \nabla \cdot \left( \alpha_l \rho_l \vec{u}_l \right)         & = & -(1-\eta)\dot{\Gamma}^{'''} - \dot{\Upsilon}^{'''} + s_{m,l} \\
\label{eqn:conservation_of_ent}
\frac{\partial \left(\alpha_e \rho_l \right)}{\partial t } + \nabla \cdot \left( \alpha_e \rho_l \vec{u}_e \right)         & = & -\eta\dot{\Gamma}^{'''} + \dot{\Upsilon}^{'''}+ s_{m,e}
\end{IEEEeqnarray}
...
The terms on the right-hand side represent the volumetric inter-field transfer ($\dot{\Upsilon}^{'''}$), inter-phase transfer ($\dot{\Gamma}^{'''}$), and external ($s_{m,k}$) sources or sinks of mass.
Since there are two liquid fields, the net phasic mass transfer between the water-vapor field and the liquid fields, $\dot{\Gamma}^{'''}$, is apportioned between the continuous liquid field and the entrained liquid field.
...
\dot{\Gamma}^{'''} h^{'}_v + q^{'''}_{i,v} + q^{'''}_{n,l}  + q^{'''}_{w,g} + \alpha_g\frac{\partial P}{\partial t} + s_{e,g}  & &\\
...
-\dot{\Gamma}^{'''} h^{'}_l +  q^{'''}_{i,l} - q^{'''}_{n,l}  + q^{'''}_{w,l} + (1 - \alpha_g) \frac{\partial P}{\partial t} + s_{e,l}  & &
\end{IEEEeqnarray}
...
\frac{\partial \left( \alpha_l \rho_l \vec{u}_l \right )}{\partial t } + \nabla \cdot \left( \alpha_l \rho_l \vec{u}_l \vec{u}_l \right) & = & \nonumber \\
 -\alpha_l \nabla P + \alpha_l \rho_l \vec{g} - \vec{\tau}^{'}_{w,l} + \vec{\tau}^{'}_{i,gl} - (1 - \eta)\dot{\Gamma}^{'''}\vec{u}^{'} - \dot{\Upsilon}^{'''}\vec{u}^{'} + s_{p,l} & & \\
...
\frac{\partial \left( \alpha_g \rho_g \vec{u}_g \right) }{\partial t } + \nabla \cdot \left( \alpha_g \rho_g \vec{u}_g \vec{u}_g \right) & = & \nonumber \\
 -\alpha_g \nabla P + \alpha_g \rho_g \vec{g} - \vec{\tau}^{'}_{w,g} - \vec{\tau}^{'}_{i,gl} - \vec{\tau}^{'}_{i,ge} + \dot{\Gamma}^{'''}\vec{u}^{'} + s_{p,g} & & \\
...
\frac{\partial \left( \alpha_e \rho_l \vec{u}_e \right) }{\partial t } + \nabla \cdot \left( \alpha_e \rho_l \vec{u}_e \vec{u}_e \right) & = & \nonumber \\
 -\alpha_e \nabla P + \alpha_e \rho_l \vec{g} - \vec{\tau}^{'}_{w,e} + \vec{\tau}^{'}_{i,ge} - \eta \dot{\Gamma}^{'''}\vec{u}^{'} + \dot{\Upsilon}^{'''}\vec{u}^{'} + s_{p,l} & &
\end{IEEEeqnarray}
...
The assumption in these integrals is that the value of the conserved quantities and all thermodynamically-related variables are constant within a given continuity volume.
...
\int_{V_j}\frac{\partial \left(\alpha_l \rho_l \right)}{\partial t } & + & \nabla \cdot \left( \alpha_l \rho_l u_l \right) \mathrm{d}V = \int_{V_j} \left(-(1-\eta)\dot{\Gamma}^{'''} - \dot{\Upsilon}^{'''} + s_{m,l}\right) \mathrm{d}V \nonumber \\
V_j \frac{\partial \left(\alpha_{l,j} \rho_{l,j} \right)}{\partial t } & = & -\int_{V_j}\nabla \cdot \left( \alpha_l \rho_l u_l \right) \mathrm{d}V -(1-\eta_j)\dot{\Gamma}_j - \dot{\Upsilon}_j + s_{m,l,j}V_j \nonumber \\
V_j \frac{\partial \left(\alpha_{l,j} \rho_{l,j} \right)}{\partial t } & = & -\left[\alpha_l \rho_l u_l A_{m}\right]_{x_{j-\onehalf}}^{x_{j+\onehalf}} -(1-\eta_j)\dot{\Gamma}_j - \dot{\Upsilon}_j + s_{m,l,j}V_j \nonumber \\
...
& & -(1-\eta)\dot{\Gamma}_j - \dot{\Upsilon}_j + s_{m,l,j}V_j
\end{IEEEeqnarray}
...
\Loop \; Transient Loop
    ...
    \For{$s = 1 \to N_{s}$} \; Stage Loop
		\BlackBox Solve $\displaystyle \frac{\vec{y}^{s} - \vec{y}^{n}}{\Delta t} =  \vec{E}_{s}^{*}$ for $\vec{y}^{s}$.
	\EndFor
	\State $n = n + 1$
\EndLoop
\end{algorithmic}
...
At each stage the choice of how to approximate the driving function, $\vec{E}_{s}^{*}$, can change in both its functional dependence upon the stage values of $\vec{y}^{s}$ and in which components of $\vec{E}^{*}$ are included.
...
Using the local fluid conditions, $\dt{}_i$ is calculated at every point within the domain.
...
For each stage the residual and Jacobian matrix will be denoted by $\vec{F}_s$ and $\vec{J}_s$, with $s = 1 \to 3$.
...
\Loop \; Transient Loop
    \Set $t^{n+1} : = t^{n} + \Delta t$
	\Calculate $\vec{F}_1$ and $\vec{J}_1$
	\Calculate $\vec{x}^{*}$ and $\vec{u}^{n+1}$ from $\vec{\delta x}^{*} = -\vec{J}^{-1}_1\vec{F}_1$
	\Calculate $\vec{F}_2$
	\Calculate $\vec{x}^{**}$ from $\vec{F}_2$
	\Calculate $\vec{x}^{n+1}$ from $\vec{F}_3$.
	\Set $n \pluseq 1$
\EndLoop
\end{algorithmic}

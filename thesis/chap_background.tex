...
The terms on the right-hand side represent the volumetric inter-field transfer ($\dot{\Upsilon}^{'''}$), inter-phase transfer ($\dot{\Gamma}^{'''}$), and external ($\dot{s}^{'''}_{m,\phi}$) sources or sinks of mass.
...
\dot{\Gamma}^{'''} h^{'}_v + \dot{q}^{'''}_{i,v} + \dot{q}^{'''}_{n,l}  + \dot{q}^{'''}_{w,g} + \alpha_g\frac{\partial P}{\partial t} + \dot{s}^{'''}_{e,g}  & &\\

...
\frac{\partial \left( \alpha_l \rho_l \vec{u}_l \right )}{\partial t } + \nabla \cdot \left( \alpha_l \rho_l \vec{u}_l \vec{u}_l \right) & = &
 ...
 & + & \vec{\tau}^{'}_{i,gl} - (1 - \eta)\Gamma^{'''}\vec{u}^{'} - S^{'''}\vec{u}^{'} + s_{p,l} \nonumber \\
...
\begin{itemize}
\item{Contain pipes}
\item{Defines spatial discretization of pipes within section.}
\item{Splitting and merging of pipes occur at section boundaries.}
\end{itemize}
}
\item{Pipes
\begin{itemize}
...
\item{Gaps
\begin{itemize}
\item{Connect pipes within a section.}
...
\begin{itemize}
\item{Develop, implement, and test a unique domain decomposition method based upon RELAP5-3D semi-implicit code coupling within COBRA.}

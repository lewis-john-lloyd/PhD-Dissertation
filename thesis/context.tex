...
After a single Newton step, the updated parameters are evaluated to determine their validity in the context of a single-shot method.
...
There are also limits placed upon the changes thermodynamic parameters within a timestep in \cobra{}.
\begin{itemize}
...
\begin{equation}
\label{eqn:time_step}
\Delta t^{n \rightarrow n+1} = \max\left[ \Delta t_{\text{MIN}}, \min\left[\frac{6}{5}\Delta t^{n-1 \rightarrow n}, \frac{5}{6}\Delta t_{\text{CRNT}}, \Delta t_{\text{MAX}} \right]\right]
\end{equation}

$\Delta t_{\text{CRNT}}$ is the most restrictive Courant number calculated for both axial and transverse flow.
In addition, there is a continuity volume based $\Delta t$ that is based upon the time calculated to empty a given volume of the mass of a given field.
The most restrictive of the Courant time-step is used as $\Delta t_{\text{CRNT}}$.
%--------------------------------------------------------------------------------------------------------------------------------------------------------------------
%--------------------------------------------------------------------------------------------------------------------------------------------------------------------
%--------------------------------------------------------------------------------------------------------------------------------------------------------------------
%--------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
This coupling technique allows for the material Courant limit to be uniformly applied between the two pieces of software.
...
This enables coupled simulations to exceed the sonic Courant limit.
...
The sonic Courant limit of explicit domain coupling led to the development of semi-implicit domain coupling.
...
However, this application does not worry about consistency at the boundaries of the subdomains since the obtained subdomain solutions are used as a preconditioned initial guess for a global Newton-Krylov-Schwarz algorithm \cite{Chan1984}.
...
\draw[pattern=north west lines, pattern color=blue] (0,-4) rectangle +(3,3);
\draw[pattern=north east lines, pattern color=red] (0.75,-3.25) rectangle +(1.5,1.5);
\draw (1.5,-1) node[anchor=south] {Proposed Method};
\end{tikzpicture}
\end{center}
\caption{Current software for thermal-hydraulic safety analysis.}
\label{fig:my_diagram}
\end{figure}

The advantages of the single-shot linearization is the reduction in computational costs; however, the accuracy in region of highly non-linear physics is suspect.
...
The objective of this research is the design, implementation, and evaluation of a novel spatially selective nonlinearly convergent solution framework for nuclear thermal hydraulic safety analysis.

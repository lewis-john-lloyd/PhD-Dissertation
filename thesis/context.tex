...
\draw (0.5,-1.5) node {\verb!Volumes!};
\draw (3.5,-1) node {\verb!Momentum!};
\draw (3.5,-1.5) node {\verb!Volumes!};
\end{tikzpicture}
\end{center}
...
It was developed to overcome the sonic Courant limitations of the explicit method while avoiding the high  computational cost of the fully implicit method \cite{Liles1978}.
...
The two paradigms of discrete software coupling and domain decomposition are two sides of the same coin.
...
Isolation of a subdomain where nonlinearities are high will be achieved by using a variant of the code coupling algorithm discussed in \sect{sect:code_coupling}.
For this work, the subdomain to be isolated will be geometric components predetermined by the user.

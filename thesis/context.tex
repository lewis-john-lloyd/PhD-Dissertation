...
All operating commercial reactors within the United States are of a vertically oriented LWR design.
...
The basis for this assumption is that while the entrained droplets are here being modeled by a separate set of governing equations than those of the continuous liquid field, in reality the droplets are constantly entraining from and depositing to the continuous liquid field. 
}
\item{
...
Recalling the staggered mesh from \sect{sect:topology}, the six scalar conservation laws, \eqref{eqn:conservation_of_ncg} -- \eqref{eqn:con_energy_liq}, are each integrated over the continuity volumes.
...
V_j \frac{\partial \left(\alpha_{l,j} \rho_{l,j} \right)}{\partial t } & = & -\left[\alpha_l \rho_l u_l A_{m}\right]_{x_{j-\frac{1}{2}}}^{x_{j+\frac{1}{2}}} -(1-\eta_j)\Gamma_j - S_j + s_{m,l,j}V_j \nonumber \\
\label{eqn:spatially_discrete_liq_m_con}
V_j \frac{\partial \left(\alpha_{l,j} \rho_{l,j} \right)}{\partial t } & = & -\left( <\alpha_l \rho_l>_{d,j+\frac{1}{2}} u_{l,j+1} A_{m,j+1} - <\alpha_l \rho_l>_{d,j-\frac{1}{2}} u_{l,j} A_{m,j}\right) \nonumber \\
...
\cobra{} possesses the physics necessary to model the complicated in-core flow patterns and heat transfer encountered during a postulated LOCA.
...
Upon formulation of the pressure matrix, the Newton update for the pressures will be expressed as a linear combination of all of the unknown mass and energy fluxes through the coupled boundaries of the domain, \eqref{eqn:si_relap} \cite{Weaver2002}.
...
The two different paradigms, discrete software coupling and domain decomposition, are two sides of the same coin.
The software coupling is the algorithmic implementation of the domain decomposition.
Using the semi-implicit software coupling as a means of domain coupling internal to a code provides a mean of isolating subdomains that provides a consistent formulation for the nonlinear problem.

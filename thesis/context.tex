...
In particular, they are a subcategory of two-phase flow \cite{Todreas2011, Stewart1984}.
...
Variants of these parameters include the use of velocities instead of momentums, temperatures instead of pressure, and internal energies instead of enthalpies \cite{RELAP, TRACE}.
...
This iteration is done by successive linearizataion of the nonlinear problem \eqref{eqn:newton_taylor}, where $\vec{J}(\vec{z})$ is the Jacobian matrix of $\vec{F}(\vec{x})$ evaluated at $\vec{z}$ and $\vec{\delta x}^k = \vec{x}^{k+1} - \vec{x}^k$.
...
The semi-implicit solution method was developed to overcome the sonic CFL limitations of the explicit method while avoiding the computational cost of the fully implicit method \cite{Liles1978}.
...
\cobra uses a single-shot linearization method to resolve the non-linearities.
...
The five step process just outlined provides an overview of the solution technique of the semi-implicit method used in \cobra.
...
Stated another way, the Jacobian for the momentum equations are fixed at the first iterate, while the Jacobians for the mass and energy equations are updated at each iterate.
The only result from stage two that is used in stage three is the final velocities, $\vec{u}^{n+1}$.
Stage three, the mass and energy equations are solved such that all continuity variables are implicit and the velocities used are those obtained from stage two.
...
The loop termination criteria is a implementation specific step.
The TRACE code uses what appears to be a $L_{inf}$ norm of the unscaled Newton updates for the continuity variables to determine convergence.

\subsection{Nearly-Implicit Method}
\label{subsect:numerics_nearly_implicit}
Another possible method used to overcome the material CFL limit without incurring the same cost as the fully-implicit method is the Nearly-Implicit method \cite{Trapp1986, RELAP}.
...
In the first stage is similar to the semi-implicit method.
...
Once the stage one velocities are obtained, the corresponding stage one continuity variables are obtained analogously to obtaining the new time momentums in the semi-implicit method.
...
Additionally, given that each of the methods above utilizes Newton's method for at least one Newton step, the globalization of the local Newton method in the context of safety analysis codes is not addressed.
...
Each of the methods outlined in \sect{sect:solution_techniques} has one thing in common: the nonlinear problem is solved globally.
...
The use of domain decomposition for the purpose of nonlinear-refinement has not been investigated in the context of thermal-hydraulic safety methods. 
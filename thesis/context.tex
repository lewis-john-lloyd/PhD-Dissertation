...
There are several codes that are widely available for simulating the thermal-hydraulic response of a nuclear power plant.
...
\comment{Here an intro is required.}
...
\comment{Discuss ill-posedness of the equations. And then mention that it doesn't matter.}

The above collection of governing equations posses a potentially troublesome property.
The assumption of pressure equilibrium between the fields produces a ill-posed collection of equations \cite{Ransom1984}.
...
For illustrative purposes, \eqref{eqn:conservation_of_liq} will be integrated over a given volume, $j$, as shown in \fig{fig:single_volume}.
...
One thing that distinguishes this the numerical method in \cobra{} from other methods is its treatment of the temporal derivative.
...
The other option, the non-conservative form, is analytically expand the temporal derivative via the chain rule, \eqref{eqn:non_conservative}, and these equations are then temporally discretized.
...
In \alg{alg:local_newton}, there are two black box calculations: the calculation of the update independent parameters, $\vec{x}^{n+1,k+1}$; and the second is the calculation of the loop termination criteria.
...
\cobra{} posses the physics necessary to model the complicated flow patterns encountered during LOCA within the core of an NPP.
However, it lacks the detailed models for NPP components (steam-generators, heat exchanges, etc) that are traditionaly available from system-analysis codes.
When modeling full NPP transients, to it is advantageous to use sub-channel software for the core and system analysis software to model the rest of the NPP.
...
There are several examples of the first method: MARS \cite{Jeong2008} and TRACE \cite{TRACE} are two examples.
...
A quick outline of the method will be now be presented.

\comment{Here is where I will talk the coupling method.}

The coupling of system analysis codes and sub-channel codes can be viewed through a different lens.
...
Within the realm of domain decomposition, the two classes of the Schwarz method will be of interest.
There is the multiplicative Schwarz and the additive Schwarz.

From there I will present the view of code coupling as one of either additive or multiplicative Schawrz applied to a larger problem with different physics applied in different areas.
...
\comment{Degree of detail fuzzy.}

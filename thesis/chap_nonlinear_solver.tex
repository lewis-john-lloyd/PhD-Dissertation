...
V_c \left[(\alpha_g \rho_{n})^{n+1}\right. & - & \left. (\alpha_g \rho_{n})^{n}\right] = -\dt{} \sum_{i\, \in \, N_{f}}\left( \don{\alpha^{n}_g \rho^{n}_{n}}^{n+1}_{d} u^{n+1}_g  A_{p} \right)_{i} \\
\label{eqn:nlnLiqMassEquation}
V_c \left[ \left(\alpha_l \rho_l \right)^{n+1} \right. & - & \left. \left(\alpha_l \rho_l \right)^{n} \right]=  -\dt{} \sum_{i\, \in \, N_{f} }\left( \don{\alpha^n_l \rho^n_l}^{n+1}_{d} u^{n+1}_l  A_{p} \right)_{i} \nonumber \\
&- &\left[(1-\eta)\Gamma + \Upsilon\right]^{n+1} \\
\label{eqn:nlnGasEnergyEquation}
V_c \left[ \left( \alpha_g \rho_g h_g \right)^{n+1} \right. & - & \left. \left( \alpha_g \rho_g h_g \right)^{n} - \alpha^{n}_g ( P^{\,n+1} - P^{n} ) \right] = \left[ q_{i,v} + \Gamma h^{'}_v + q_{gl} \right]^{n+1} + q^{n}_{wg}\nonumber \\
& - &\dt{} \sum_{i\, \in \, N_{f}} \left(  \don{\alpha^{n}_g \rho^{n}_g h^{n}_{g}}^{n+1}_{d} u^{n+1}_g  A_{p} \right)_{i} \\ 
\label{eqn:nlnLiqEnergyEquation}
V_c\left[\left( \alpha_l \rho_l h_l \right)^{n+1} \right. & - & \left. \left( \alpha_l \rho_l h_l \right)^{n} - \alpha^{n}_l (P^{\,n+1} - P^{\,n}) \right] = \left[q_{i,l} -\Gamma h^{'}_l - q_{gl}\right]^{n+1}  + q^{n}_{wl} \nonumber \\
& -& \dt{} \sum_{i\, \in \, N_{f} } \left( \don{\alpha^{n}_l \rho^{n}_l h^{n}_l}^{n+1}_{d} u^{n+1}_l A_{p} + \don{\alpha^{n}_e \rho^{n}_l h^{n}_l}^{n+1}_{d} u^{n+1}_e  A_{p} \right)_{i}\\
\label{eqn:nlnEntMassEquation}
V_c \left[ \left(\alpha_e \rho_l \right)^{n+1}\right. & - & \left. \left(\alpha_e \rho_l \right)^{n} \right]= -\dt{} \sum_{i\,\in \,N_{f}} \left( \don{\alpha^{n}_e \rho^{n}_l}^{n+1}_{d} u^{n+1}_e  A_{p} \right)_{i}+ \left[ \Upsilon -\eta\Gamma \right]^{n+1} \\
\label{eqn:nlnVapMassEquation}
V_c\left[ \left(\alpha_g \rho_v \right)^{n+1}\right. &-& \left. \left(\alpha_g \rho_v \right)^{n}\right] = - \dt{} \sum_{i \, \in \, N_{f}} \left( \don{\alpha^{n}_g \rho^{n}_v}^{n+1}_{d} u^{n+1}_g  A_{p} \right)_{i} + \Gamma^{n+1}
\end{IEEEeqnarray}
...
Note that a discussion of vectors $\vec{F}_{p}$ and $\vec{F}_{c}$ will be in \sect{subsect:nlnCobraAlgo}, and one of vectors $\vec{S}_{p}$ and $\vec{S}_{c}$ will be in \sect{sect:nlnScaling}.
...
As mentioned in \sect{sect:geometry} subchannels inherit their geometric properties from the section in which they reside.
...
\fig{fig:activeDomain} shows an example of a domain with inactive regions, where the inactive portions of the domain are marked with grey, the active with white, the external boundary conditions with black, and the inter-subchannel flow paths are marked with dashed lines.
...
During this research, the \cobra{} software was transitioned from being restricted to using only a linear solver, as outlined in \sect{sect:linCobraAlg}, to being able to use either a linear solver or a nonlinearly convergent solver. 
...
Lastly, the methods by which convergence is determined are discussed and presented in \alg{alg:nlnConvergence}.
...
\vec{J}^{k}_{p} \delta \momVec{}^{k} & = & -\vec{F}_{p}^{k} - \sum_{i\,\in \, N_{c} } \frac{\partial \vec{F}^{k}_{p}}{\partial P_{i}} \delta P^{k}_{i} \nonumber \\
\vec{J}^{k}_{p} \left[\momVec{}^{n+1, k + 1} - \momVec{}^{n+1, k} \right] & = & -\vec{F}^{k}_{p} - \sum_{i \, \in \, N_{c} } \frac{\partial \vec{F}^{k}_{p}}{\partial P_{i}} \delta P^{k}_{i} \nonumber \\
\vec{J}^{k}_{p} \momVec{}^{n+1, k + 1} & = & - \vec{F}^{k}_{p} + \vec{J}^{k}_{p} \momVec{}^{n+1, k} - \sum_{i \, \in \, N_{c} } \frac{\partial \vec{F}^{k}_{p}}{\partial P_{i}} \delta P^{k}_{i} \nonumber \\
\momVec{}^{n+1, k + 1} & = & \left[\vec{J}_{p}^{k}\right]^{-1} \left[- \vec{F}^{k}_{p}  + \vec{J}^{k}_{p} \momVec{}^{n+1, k} \right] - \sum_{i \, \in \, N_{c} } \left[\vec{J}^{k}_{p}\right]^{-1} \left[\frac{\partial \vec{F}^{k}_{p}}{\partial P_{i}}\right] \delta P^{k}_{i} \nonumber \\
...
\begin{IEEEeqnarray}{rcl}
\label{eqn:momHalf}
\momVec{}^{n+1, k + \onehalf} & = & \left[\vec{J}^{k}_{p}\right]^{-1}\left[ \vec{J}^{k}_{p} \momVec{}^{n+1, k}  - \vec{F}^{k}_{p} \right] \nonumber \\
\momVec{}^{n+1, k + \onehalf} & = & \momVec{}^{n+1, k} - \left[\vec{J}^{k}_{p}\right]^{-1}\vec{F}^{k}_{p} \nonumber \\
...
\delta \momVec{}^{k} & = & - \left[\vec{J}_{p}^{k}\right]^{-1} \vec{F}^{k}_{p} - \sum_{i\,\in \, N_{c}} \left[\vec{J}_{p}^{k}\right]^{-1}\left[\frac{\partial\, \vec{F}^{k}_{p}}{\partial P_{i}}\right] \delta P_{i}^{k} \nonumber \\
...
& - & \left[q_{i,v} + \Gamma h^{'}_v + q_{gl}\right]^{n+1, k} + \dt{} \sum_{i\,\in\,N_{f}} \left(\don{\alpha^{n}_g \rho^{n}_g h_g^{n}}^{n+1,k}_{d} u^{n+1, k}_g  A_{p} \right)_{i} \\
...
& +& \dt{} \sum_{i \, \in \, N_{f} } \left( \don{\alpha^{n}_l \rho^{n}_l h^{n}_l}^{n+1,k}_{d} u^{n+1,k}_l A_{p} + \don{\alpha^{n}_e \rho^{n}_l h^{n}_l}^{n+1,k}_{d} u^{n+1,k}_e  A_{p} \right)_{i} - q^{n}_{wl}\\
...
\frac{\partial \vec{F}^{k}_{c}}{\partial \alpha_{e}} \delta \alpha_{e}^{k} + \frac{\partial \vec{F}^{k}_{c}}{\partial P } \delta P^{k} + \dt{} \sum_{i\, \in \, N_{f} } \vec{\Xi}^{k}_{i}\left( \delta \momVec{}_{i}^{*} + \frac{\partial \momVec{}_{i}^{k}}{\partial P_{s(i)}} \delta P_{s(i)}^{k} + \frac{\partial \momVec{}_{i}^{k}}{\partial P_{o(i)}} \delta P_{o(i)}^{k} \right) = \vec{F}^{k}_{c} & &
\end{IEEEeqnarray}
...
\frac{\partial \vec{F}^{k}_{c}}{\partial (\alpha_{g} P_{n} )} \delta (\alpha_{g} P_{n})^{k} + \frac{\partial \vec{F}^{k}_{c}}{\partial \alpha_{g}} \delta \alpha^{k}_{g} + \frac{\partial \vec{F}^{k}_{c}}{\partial (\alpha_{g} h_{v} )} \delta (\alpha_{g} h_{v})^{k} + \frac{\partial \vec{F}^{k}_{c}}{\partial ((1 - \alpha_{g}) h_{l} )} \delta ((1 - \alpha_{g}) h_{l})^{k} & + & \nonumber \\
\frac{\partial \vec{F}^{k}_{c}}{\partial \alpha_{e}} \delta \alpha_{e}^{k} + \left( \frac{\partial \vec{F}^{k}_{c}}{\partial P } + \dt{} \sum_{i\,\in \, N_{f} } \vec{\Xi}^{k}_{i}\frac{\partial \momVec{}^{k}_{i}}{\partial P_{s(i)}}\right) \delta P^{k} + \dt{} \sum_{i \, \in \, N_{f} } \vec{\Xi}^{k}_{i} \frac{\partial \momVec{}_{i}^{k}}{\partial P_{o(i)}} \delta P_{o(i)}^{k} & = & \nonumber \\
- \vec{F}^{k}_{c} - \dt{} \sum_{i \, \in \, N_{f} } \vec{\Xi}^{k}_{i} \delta \momVec{}_{i}^{*} & &
\end{IEEEeqnarray}
...
S^{k}_{p, l} & = & \frac{\dt{}}{\dx{}} \left[\abs{\frac{\dx{} \left[\dot{m}_{l}^{n+1, k} - \dot{m}_{l}^{n}\right]}{\dt{}}} + \abs{\sum_{i\, \in \, N_{c} } \left( \don{\alpha_l \rho_l u_l}_{\text{d}} \ave{u}_{\text{a},l} \tilde{A}\right)^{n}_{i}} +\abs{\ave{\alpha_l}^{n}_{\text{a}} \nabla P^{\,n+1, k}} + \abs{g \ave{\alpha_l \rho_l}_{\text{a}}^{n}} \right.\nonumber \\
...
S^{k}_{p, g} & = & \frac{\dt{}}{\dx{}} \left[\abs{\frac{\dx{} \left[ \dot{m}_{g}^{n+1,k} - \dot{m}_{g}^{n}\right]}{\dt{}}} + \abs{\sum_{i\, \in \, N_{c} } \left( \don{\alpha_g \rho_g u_g}_{\text{d}} \ave{u}_{\text{a},g}  \tilde{A}\right)_{i}^{n}}  +\abs{\ave{\alpha_g}^{n}_{\text{a}} \nabla P^{\,n+1, k}} + \abs{g \ave{\alpha_g \rho_g}_{\text{a}}^{n}} \right. \nonumber \\
...
S^{k}_{p, e} & = & \frac{\dt{}}{\dx{}} \left[\abs{\frac{\dx{} \left[\dot{m}_{e}^{n+1, k} - \dot{m}_{e}^{n}\right]}{\dt{}}} + \abs{\sum_{i\, \in \, N_{c}} \left( \don{\alpha_e \rho_l u_e}_{\text{d}} \ave{u}_{\text{a},e}  \tilde{A}\right)_{i}^{n}} + \abs{\ave{\alpha_{e}}^{n}_{\text{a}} \nabla P^{\,n+1, k}} + \abs{g \ave{\alpha_e \rho_l}^{n}_{\text{a}}} \right. \nonumber \\
...
Likewise, the scale factors for the continuity equations, \eqref{eqn:nlnNcgMassEquation} -- \eqref{eqn:nlnVapMassEquation}, are given in  \eqref{eqn:nlnNcgMassScale} -- \eqref{eqn:nlnVapMassScale}.

\begin{IEEEeqnarray}{lCl}
\label{eqn:nlnNcgMassScale}
S^{k}_{m, n} & = & \dt{}\left[\abs{\frac{V_c\left[(\alpha_g \rho_{n})^{n+1, k} -(\alpha_g \rho_{n})^{n}\right]}{\dt{}}} + \sum_{i\,\in \, N_{f}}\abs{ \don{\alpha^{n}_g \rho^{n}_{n}}^{n+1,k}_{d} u^{n+1, k}_{g}  A_{p} }_{i} \right]\\
\label{eqn:nlnLiqMassScale}
S^{k}_{m, l} & = & \dt{}\left[\abs{\frac{V_c \left[\left(\alpha_l \rho_l \right)^{n+1,k} - \left(\alpha_l \rho_l \right)^{n}\right]}{\dt{}}} + \sum_{i\, \in \, N_{f} } \abs{ \don{ \alpha^n_l \rho^n_l }^{n+1,k}_{d} u^{n+1, k}_l A_{p} }_{i} \right. \nonumber \\
...
S^{k}_{h, g} & = & \dt{}\left[\abs{\frac{V_c \left[\left( \alpha_g \rho_g h_g \right)^{n+1, k} - \left( \alpha_g \rho_g h_g \right)^{n}\right]}{\dt{}}} +\abs{\frac{V_c \alpha^{n}_{g} ( P^{n+1, k} - P^{n} )}{\dt{}}} + \abs{\dot{q}_{wg}^{n}} \right. \nonumber \\
& + & \left. \abs{\left[\dot{\Gamma} h^{'}_v\right]^{n+1,k}} + \abs{\dot{q}_{i,v}^{n+1,k}} +\abs{\dot{q}^{n+1,k}_{gl}} + \sum_{i\,\in \, N_{f} } \abs{\don{\alpha^{n}_g \rho^{n}_g h_g^{n}}^{n+1,k}_{d} u^{n+1, k}_g  A_{p}}_{i} \vphantom{\abs{\frac{V_c \left[\left( \alpha_g \rho_g h_g \right)^{n+1, k} - \left( \alpha_g \rho_g h_g \right)^{n}\right]}{\dt{}}}} \right]\\
\label{eqn:nlnLiqEnergyScale}
S^{k}_{h, l} & = & \dt{}\left[ \abs{\frac{V_c\left[\left( \alpha_l \rho_l h_l \right)^{n+1,k} - \left( \alpha_l \rho_l h_l \right)^{n}\right]}{\dt{}}} + \abs{\frac{V_c \alpha^{n}_l (P^{\,n+1,k} - P^{\,n})}{\dt{}}} + \abs{\dot{q}_{wl}^{n}} \right . \nonumber \\
& + & \abs{\left[\dot{\Gamma} h^{'}_l\right]^{n+1}} + \abs{\dot{q}^{n+1,k}_{i,l}} + \abs{\dot{q}_{gl}^{n+1,k}} + \sum_{i\,\in \, N_{f} }\abs{ \don{ \alpha^{n}_l \rho^{n}_l h^{n}_l }^{n+1,k}_{d} u^{n+1,k}_l A_{p} }_{i} \nonumber \\
& + & \left. \sum_{i\,\in \, N_{f} } \abs{ \don{ \alpha^{n}_e \rho^{n}_l h^{n}_l }^{n+1,k}_{d} u^{n+1,k}_e  A_{p} }_{i} \vphantom{ \abs{\frac{V_c\left[\left( \alpha_l \rho_l h_l \right)^{n+1,k} - \left( \alpha_l \rho_l h_l \right)^{n}\right]}{\dt{}}}} \right] \\
\label{eqn:nlnEntMassScale}
S^{k}_{m, e} & = & \dt{} \left[ \abs{\frac{V_c \left[\left(\alpha_e \rho_l \right)^{n+1,k} - \left(\alpha_e \rho_l \right)^{n}\right]}{\dt{}}} + \sum_{i\,\in \, N_{f}}\abs{\left( \don{\alpha^{n}_e \rho^{n}_l}^{n+1, k}_{d} u^{n+1,k}_e  A_{m} \right)}_{i} \right. \nonumber \\
...
S^{k}_{m, v} & = & \dt{}\left[\abs{\frac{V_c \left[\left(\alpha_g \rho_v \right)^{n+1, k} - \left(\alpha_g \rho_v \right)^{n}\right]}{\dt{}}} + \sum_{i\,\in \, N_{f}} \abs{ \don{ \alpha^{n}_g \rho^{n}_v }^{n+1,k}_{d} u^{n+1, k}_{g}  A_{p} }_{i} \right. \nonumber \\
...
This particular phase-transition scaling produced the regular scaling factor when the volume fraction of a phase was at least two orders of magnitude greater than the minimum volume fraction for that phase or field, which is on the order of $\expneg{1.0}{6}$.
...
%Unfortunately, precise measurement of the error in a simulation requires that an analytic solution be available for comparison.
...
%R = \int_{t^{0}}^{t^{N_{t}}} ||\tilde{\vec{F}}(\tau)||_2 \,\mathrm{d} \tau
%\end{equation}
%
%Given the bounds of the scaled residual it was considered desirable to have a similarly scaled transient residual.
%The transient residual in \eqref{eqn:metricResidualIntegral} possesses a dependence upon the length of the transient.
...
%\tilde{R} = \frac{\int_{t^{0}}^{t^{N_{t}}} ||\tilde{\vec{F}}(\tau)||_2 \,\mathrm{d} \tau}{t^{N_{t}} - t^{0}}
%\end{equation}
%
...
%Since different simulations may have different transient behavior, fixing the moment arm at $t^{0}$ is too strict.
%
%\begin{equation}
%\label{eqn:metricResidualMoment}
%\tilde{R}_{\text{M}} = \frac{\int_{t^{0}}^{t^{N_{t}}} \,\tau\,||\tilde{\vec{F}}(\tau)||_2 \,\mathrm{d} \tau}{\int_{t^{0}}^{t^{N_{t}}} \,\tau \,\mathrm{d} \tau}
%\end{equation}
...
Third, several necessary and practical considerations required to allow the software to simulate the transition from single-phase to multi-phase flow are outlined.
...
The presence of the nonlinear input file, ``\classname{nwt.cob}," triggers the input processing routine for the nonlinear solver.
...
The constrained thermodynamic parameters and the limits imposed upon them are:
...
\dtcrnt{} is the most restrictive of $\dt{}_{\text{vol}}$ and the material Courant limit from both axial and transverse flows.
$\dt{}_{\text{vol}}$ is the most restrictive timestep based upon an estimation of the time required to remove all of the mass from a given continuity volume.

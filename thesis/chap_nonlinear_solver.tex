...
\Loop \; Transient Loop
    ...
    \State \; $k = 0$
    \Loop \; Newton Loop
	   	\Loop \; Nonlinear Momentum Loop
   			\Calculate $\dot{\vec{m}}^{*}$ and $\frac{\partial \dot{\vec{m}}}{\partial P}$   		
	   	\EndLoop
	   	\Loop \; Noninear Mass \& Energy Loop
   			\Calculate $\vec{J}$
   			\State Assemble $\vec{A}$ and $\vec{b}$
	   	\EndLoop   	
   		\State Solve $\vec{A} \delta P = \vec{b}$
		\Loop \; Update Mass \& Energy Loop
			\State Apply $\vec{U}^{-1}_{j}$ to obtain $\delta \vec{C}_{j}$
		\EndLoop
	\EndLoop
	\State $n = n + 1$
\EndLoop

...
The momentum equations are then linearized around the current time value, $\dot{\vec{m}}^{n}$ and $P^{n}$, giving \eqref{eqn:linLiqMomentumEquation} - \eqref{eqn:linEntMomentumEquation}.
...
The resulting system is of the form $\displaystyle \vec{J}_{m, j} \dot{\vec{m}}_{j}^{n+1} + \sum^{N_{c}}_{i\,=\,1} \vec{b}_{j} \delta P_{i} = \vec{a}_{j}$.
The solution of this linear system is expressed in \eqref{eqn:momStar}.

\begin{equation}
\label{eqn:momStar}
\dot{\vec{m}}_{j}^{n+1} + \sum^{N_{c}}_{i\,=\,1} \frac{\partial \dot{\vec{m}}_{j}}{\partial P_{i}} \delta P_{i} = \dot{\vec{m}}^{*}_{j}
\end{equation}
...
As obtained, the \cobra{} software used the single timestep of the semi-implicit method as outlined in \sect{sect:linCobraAlg}.
...
\frac{\partial\, \vec{F}_{m}}{\partial \dot{m}^{n+1}_{l}} \delta \dot{m}_{l}^{k} + \frac{\partial\, \vec{F}_{m}}{\partial \dot{m}^{n+1}_{g}} \delta \dot{m}_{g}^{k} + \frac{\partial\, \vec{F}_{m}}{\partial \dot{m}^{n+1}_{e}} \delta \dot{m}_{e}^{k} + \sum_{i\,=\,1}^{N_{c}} \frac{\partial\, \vec{F}_{m}}{\partial P_{i}} \delta P_{i}^{k} = - \vec{F}_{m}
\end{equation}

This system of equations is solved for the new-time momenta.
...
In \eqref{eqn:linSystem}, the matrix $\vec{J}_{c, j}$ is the Jacobian of the six continuity equations with respect to the six independent parameters for those equations, as shown in \eqref{eqn:jacobianWithFluxTerms}.
...
The algorithm for a single Newton step is given by \alg{alg:xschem}.
...
The single Newton step outlined in \sect{sect:linCobraAlg} is used to construct the pressure matrix for the nonlinear solver.
During the update portion of the nonlinear solver, \alg{alg:updateVariables}, the new-time momentum variables are determined based upon \eqref{eqn:momStar}.
For the nonlinear solver, the change in momenta, $\delta \vec{\dot{m}}$, is calculated for each flowpath, \eqref{eqn:deltaMomenta}.
...
\delta \vec{\dot{m}}^{k}_{j} = \vec{\dot{m}}^{n+1,k,*}_{j} + \sum^{N_{c}}_{i\,=\,1} \frac{\partial \vec{\dot{m}}_{j}}{\partial P_{i}}\big|^{n+1,k} \delta P^{k}_{i} - \vec{\dot{m}}^{n}_{j}
\end{equation}

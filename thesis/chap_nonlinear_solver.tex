...
During this process both the residuals and the scale factors, $\vec{F}^{k}$ and $\vec{S}^{k}$, are evaluated and stored.
During this Newton step the pressure matrix is both constructed and solved.
During the update portion of the nonlinear solver, \alg{alg:updateVariables}, the new-time momentum variables are calculated using \eqref{eqn:momentumSolution}.
...
Note that is assembled and stored during the process of assembling the pressure matrix, \alg{alg:xschem}.

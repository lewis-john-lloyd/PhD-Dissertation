...
Second, the process through which the Newton vector is updated is outlined and discussed.
Lastly, the method by which determination of convergence is discussed.
...
Two quantities that are important to the nonlinear solver are the residual vector, $\vec{F}^{k}$, and the update vector, $\delta \vec{x}$.
The residual vector is composed of the residuals from the governing partial differential equations that are being solved.
The update vector represents that changes in the solution vector that result from a given Newton step.
...
\momVec{}^{n+1, k + 1} & = & \left[\vec{J}^{k}\right]^{-1} \left[- \vec{F}^{k}_{m}  + \vec{J}^{k}_{m} \momVec{}^{n+1, k} \right] - \sum^{N_{c}}_{i\,=\,1} \left[\vec{J}^{k}_{m}\right]^{-1} \left[\frac{\partial \vec{F}^{k}_{m}}{\partial P_{i}}\right] \delta P^{k}_{i} \nonumber \\
...
By rearranging \eqref{eqn:continuitySystem}, the following system is obtained.
...
%This matrix is show in \eqref{eqn:jacobianWithFluxTerms}.
%
%\begin{equation}
...
During this process the residuals and scale factors, $\vec{F}^{n+1,k}$ and $\vec{S}^{n+1,k}$, are evaluated and stored.

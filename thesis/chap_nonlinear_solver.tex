...
\dot{m}_{l}^{n+1} - \dot{m}_{l}^{n} & = & \frac{\Delta t}{\Delta x}\left(- \sum_{N_{k}} \left( (\alpha_l \rho_l u_l) u_l \cdot \vec{\bar{A}}\right)^{n}
 -\alpha^{n}_l \nabla P^{n+1} + (\alpha_l \rho_l)^{n} g - K^{n}_{wl}(\dot{m}_l^{n+1})^2 \right. \nonumber \\
 & + & \left. K^{n}_{i,gl}(\dot{m}^{n+1}_l - \dot{m}_g^{n+1})^2 - ((1 - \eta)\Gamma u^{'})^{n} - (S u^{'})^{n}\vphantom{\sum_{N_{k}}}\right) \\
\label{eqn:si_nln_mom_gas}
\dot{m}_{g}^{n+1} - \dot{m}_{g}^{n} & = & \frac{\Delta t}{\Delta x}\left(- \sum_{N_{k}} \left( (\alpha_g \rho_g u_g) u_g  \cdot \vec{\bar{A}}\right)^{n}  -\alpha^{n}_g \nabla P^{n+1} + (\alpha_g \rho_g)^{n} g - K^{n}_{wg}(\dot{m}_g^{n+1})^2 \right.\nonumber \\
& - & \left. K^{n}_{i,gl}(\dot{m}^{n+1}_l - \dot{m}_g^{n+1})^2 -K^{n}_{i,ge}(\dot{m}^{n+1}_e - \dot{m}_g^{n+1})^2 + (\Gamma u^{'})^{n}\vphantom{\sum_{N_{k}}}\right) \\
\label{eqn:si_nln_mom_ent}
\dot{m}_{e}^{n+1} - \dot{m}_{e}^{n} & = & \frac{\Delta t}{\Delta x}\left(- \sum_{N_{k}} \left( (\alpha_e \rho_l u_e) u_e  \cdot \vec{\bar{A}}\right)^n -\alpha^{n}_e \nabla P^{n+1} + (\alpha_e \rho_l)^{n} g - K^{n}_{we}(\dot{m}_e^{n+1})^2\right. \nonumber \\
&+& \left. K^{n}_{i,ge}(\dot{m}^{n+1}_e - \dot{m}_g^{n+1})^2 - (\eta \Gamma u^{'})^{n} + (S u^{'})^n\vphantom{\sum_{N_{k}}}\right)
\end{IEEEeqnarray}
...
\dot{m}_{l}^{n+1} - \dot{m}_{l}^{n} & = & \frac{\Delta t}{\Delta x}\left(- \sum_{N_{k}} \left( (\alpha_l \rho_l u_l) u_l \cdot \vec{\bar{A}}\right)^{n}
 ...
 & + & \left. 2 K^{n}_{i,gl}(\dot{m}^{n}_l - \dot{m}_g^{n})(\dot{m}^{n+1}_l - \dot{m}_g^{n+1}) - ((1 - \eta)\Gamma u^{'})^{n} - (S u^{'})^{n}\vphantom{\sum_{N_{k}}}\right) \\
\label{eqn:si_lin_mom_gas}
\dot{m}_{g}^{n+1} - \dot{m}_{g}^{n} & = & \frac{\Delta t}{\Delta x}\left(- \sum_{N_{k}} \left( (\alpha_g \rho_g u_g) u_g \cdot \vec{\bar{A}}\right)^{n}
 ...
 & + & \left.  (\Gamma u^{'})^{n}\vphantom{\sum_{N_{k}}}\right) \\
\label{eqn:si_lin_mom_ent}
\dot{m}_{e}^{n+1} - \dot{m}_{e}^{n} & = & \frac{\Delta t}{\Delta x}\left(- \sum_{N_{k}} \left( (\alpha_e \rho_e u_e) u_e \cdot \vec{\bar{A}}\right)^{n}
 ...
 & + & \left. 2 K^{n}_{i,ge}(\dot{m}^{n}_e - \dot{m}_g^{n})(\dot{m}^{n+1}_e - \dot{m}_g^{n+1}) - (\eta \Gamma u^{'})^{n} + (S u^{'})^{n}\vphantom{\sum_{N_{k}}}\right)
\end{IEEEeqnarray}
...
V_c (\alpha_g \rho_{n})^{n+1} & - & V_c (\alpha_g \rho_{n})^{n} = -\Delta t \sum_{N_{k}}\left( \don{\alpha^{n}_g \rho^{n}_{n}}^{n}_{d} u^{n+1}_g  \cdot \vec{\bar{A}}\right) \\
\label{eqn:si_mass_vap}
V_c \left(\alpha_g \rho_v \right)^{n+1} &-& V_c \left(\alpha_g \rho_v \right)^{n} = - \Delta t \sum_{N_{k}} \left( \don{\alpha^{n}_g \rho^{n}_v}^{n}_{d} u^{n+1}_g  \cdot \vec{\bar{A}}\right) + 
\Delta t \Gamma^{n+1} \\
\label{eqn:si_nrg_gas}
V_c \left( \alpha_g \{\rho_g h_g\} \right)^{n+1} & - & V_c \left( \alpha_g \{\rho_g h_g\} \right)^{n}  = -\Delta t \sum_{N_{k}} \left(  \don{\alpha^{n}_g \{\rho_g h_g\}^{n}}^{n}_{d} u^{n+1}_g  \cdot \vec{\bar{A}}\right) \nonumber \\
...
& -& \Delta t \sum_{N_{k}} \left( \don{\alpha^{n}_l \rho^{n}_l h^{n}_l}^{n}_{d} u^{n+1}_l \cdot \vec{\bar{A}} + \don{\alpha^{n}_e \rho^{n}_l h^{n}_l}^{n}_{d} u^{n+1}_e  \cdot \vec{\bar{A}}\right) \\
\label{eqn:si_mass_ent}
V_c \left(\alpha_e \rho_l \right)^{n+1} & - & V_c \left(\alpha_e \rho_l \right)^{n} = -\Delta t \sum_{N_{k}}\left( \don{\alpha^{n}_e \rho^{n}_l}^{n}_{d} u^{n+1}_e  \cdot \vec{\bar{A}}\right)\nonumber \\ 
&+& \Delta t\left[ S -\eta\Gamma \right]^{n+1} \\
\label{eqn:si_mass_liq}
V_c \left(\alpha_l \rho_l \right)^{n+1} & - & V_c \left(\alpha_l \rho_l \right)^{n} =  -\Delta t \sum_{N_{k}}\left( \don{\alpha^n_l \rho^n_l}^{n}_{d} u^{n+1}_l  \cdot \vec{\bar{A}}\right) \nonumber \\
...
\comment{I need to straighten out the nomenclature for connections. $N_{k}$ has previously been used to indicate the number of volumes to which a flowpath is connected. That doesn't make sense though since each flowpath can connect to only two volumes. Anyways... That needs to be fixed.}
...
\subsection{Dynamic Pressure Matrices}
...
\comment{Here is where I will layout the actual nonlinear approach.
I don't know if it is worth rehashing the same equations.
But, I think that I need to show them and talk about their time levels.
This caused a headache for me programming.
Once the equations are laid out, I will talk about how the same loop is applied.
I will then talk about the scale factor briefly as in "look later``.
Then I will layout the algorithm for checking if the solution is converged.
After the talk about convergence and its algorithm, then I will talk about undoing a Newton step and what that means.
The final algorithm will be after that.
}
...
\Loop \; Transient Loop
    ...
    \Loop \; Newton Loop
		...
		\If{ \textbf{end} Newton loop}
			\Algorithm Undo Partial Newton Step \Comment{\alg{alg:undoNewtonStep}}
...
\comment{ Here will be another algorithm that has the globalization loop.}

...
This difference is what drive the liquid's phase change into vapor.
...
The model is a simplified PWR core model with upper-head safety injection that is going through the refill phase of an accident scenario.
...
This problem is a model of a generic PWR nuclear reactor vessel.
...
The second section contains a total of five subchannels: two subchannels representing the active core, one subchannel representing the core bypass flow paths, one representing the reflector, and two subchannels representing the downcomer.
...
And, a third represents the flow bypass paths.
...
In this model, there are transverse flow paths between all adjacent subchannels in the upper head representing leakage past the hot leg nozzle seals.
However, the transverse flow paths connecting these outer ring subchannels to the two downcomer subchannels are modeled by a specialized set of PDEs meant to represent leakages paths.
These equations a specialized physical model and use a homogeneous-flow assumption.
...
This simulation is assumed to start after the blowdown phase of a LOCA.
...
This 20 [s] initialization period allows the initialization conditions to to stabilize to prevent initial stagnant conditions from impacting the transient solution.
...
First, the safety injection nozzle starts to condense the steam present in the core, dropping the pressure in the upper head.
This drop in pressure starts to draw a mixture of liquid, air, and steam into the core from the downcomer.
Once the steam generation is initiated, the water seal in the inlet plenum prevents the steam from escaping to containment and the pressure in the upper head increases, causing the liquid water to be pushed out the bottom of the core and back up into the downcomer.
There is some manometer type oscillations between the downcomer and the core as they fill with water.
The pressurization of the upper head from prevents the core filling in parallel with downcomer.
The downcomer will fill entirely while the upper head is still pressurized with steam.
Concurrent with the filling of the downcomer, the steam flow rate continues to decrease.
Eventually, the remaining steam is condensed by the safety injection water.
...
Next, \fig{fig:refillGammaNln} contains the condensation for the four different \dtmax{} using the nonlinear solver for the entire solution.
...
These results, while not as consistent over the entire range of timesteps analyzed as those produced by the global nonlinear solver, are both more consistent and more accurate than those produced by the linear solver.

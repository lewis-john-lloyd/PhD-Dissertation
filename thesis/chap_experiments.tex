...
\tab{tab:dualInlet} describes the inlet boundary condition for the two problems.
...
\caption{Residual of the flashing solution for the linear solver. solver timestep size insensitive flashing solution.}
...
The reduction in the residual exhibited by the solution produced by the linear solver, \fig{fig:flashingResidualLin}, shows that the reduction of the maximum allowable timestep size now serves two purposes.
...
There is an inlet plenum that is opens into a nine-rod bundle depicted in \fig{fig:channel_layout}.
...
\caption{Comparison of \dt{} and \dtcrnt{} for the nonlinear filling problem with \dtmax{} = \expneg{1.0}{1}{[s]}.}
...
This problem is a rough model of an nuclear reactor core.
...
The lower plenum and the downcomer are initially full of saturated water while the core region is full of saturated steam.
The water injection is located above one side of the outer ring of the reactor core.
To simulate the production of steam within the reactor core, a source of steam is introduced int the reactor core near the top of the heated region of the core.
This steam will rise into the upper head and condensation will occur as a result of the injection.
...
Additionally, the nonlinear convergence parameters \kmax{}, $f_{tol}$, and $d_{tol}$ were 35, \expneg{1.0}{6}, and \expneg{1.0}{8}.
...
\begin{figure}[h!t]
\centering
\input{plots/refloodGammaNln}
\caption{Condensation in upper head as calculated by the nonlinear solver.}
\label{fig:refloodGammaNln}
\end{figure}

Next, \fig{fig:refloodGammaNln} contains condensation rates for the four different \dtmax{}.
This axes of this graph is the same as those in \fig{fig:refloodGammaLin}.
...
\begin{figure}[h!t]
\centering
\input{plots/refloodRunTimes}
\caption{Reflood run times.}
\label{fig:refloodRunTimes}
\end{figure}

Next, the computational cost of the different methods is compared in \fig{fig:refloodRunTimes}.
...
\begin{figure}[h!t]
\centering
\input{plots/refloodRunTimeRatios}
\caption{Ratios of nonlinear and dual-domain runtimes to linear run time.}
\label{fig:refloodRunTimeRatios}
\end{figure}

These results are more accurately displayed in \fig{fig:refloodRunTimeRatios}.

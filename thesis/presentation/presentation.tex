...
\left[ \vec{U}_{c} \vert \vec{L}^{-1}_{c}\vec{K}_{c} \vert \vec{L}^{-1}_{c}\vec{Q}_{c} \right] \delta \vec{C}_{j} & = & \vec{L}^{-1}_{c}\vec{r}_{c}  -\vec{L}^{-1}_{c}\vec{Q}_{c}\vec{\Psi}_{c} \nonumber 
\end{IEEEeqnarray}
...
\centering
\vspace{0.1in}
SNR solver provides a decrease of 50\% to 60\% in run time with reduction in timestep size. 
\vspace{0.1in}
\end{textblock}
}

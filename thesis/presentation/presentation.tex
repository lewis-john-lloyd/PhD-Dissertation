...
\begin{IEEEeqnarray}{rCl}
V_c \left[ (\alpha_l \rho_l )^{n+1} \right. & - & \left. (\alpha_l \rho_l )^{n} \right] =  -\Delta t \sum_{NK}\left( <\alpha^n_l \rho^n_l>^{n}_{d} u^{n+1}_l \tilde{A} \right) \nonumber \\
&- & \left[ (1-\eta)\Gamma + S \right]^{n+1} \nonumber
\end{IEEEeqnarray}
...
\dx{} ( \dot{m}_{l}^{n+1} & - & \dot{m}_{l}^{n} ) = - \dt{} \sum_{NK} \left( <\alpha_l \rho_l u_l>^n_d u^n_l \cdot \vec{\bar{A}}\right) \nonumber \\
 ...
 & + & \left. K^{n}_{i,gl}(\dot{m}^{n+1}_l - \dot{m}_g^{n+1})^2\right] - \left[((1 - \eta)\Gamma u^{'})^{n} - (S u^{'})^{n}\right] \nonumber
\end{IEEEeqnarray}
...
\begin{IEEEeqnarray}{rcl}
S_{m,l} & = & V_c \abs{\left(\alpha_l \rho_l \right)^{n+1} - \left(\alpha_l \rho_l \right)^{n}} + \dt{} \sum_{NK}\abs{\left( <\alpha^n_l \rho^n_l>^{n}_{d} u^{n+1}_l  \tilde{A}\right)} \nonumber \\
